...
function txToSendResponse(
  ...
  return {
    ...
    claimUrl: tx.escrowRef
      ? `${process.env.APP_URL}/claim/${tx.escrowRef}`
      : null,
...
sendRouter.get("/verify-recipient", async (c) => {
  ...
  if (tumaUser?.walletAddress) {
    ...
    console.warn(
      "[Send] TUMA user name lookup unavailable — no stored display name",
    );
...
sendRouter.post(
  ...
  async (c) => {
    ...
    const lockKey = await acquireIdempotencyLock(
      "send",
      userId,
      idempotencyKey,
    );
    if (idempotencyKey && !lockKey) {
      ...
      throw new ConflictError(
        "A send with this idempotency key is already processing.",
      );
    }
    ...
    try {
      ...
      // applies to every send, direct or escrow, since both require the
      // relayer to pay real gas — and only when there's a treasury address to
      // sweep it to (otherwise it'd be "charged" without ever actually being
      // collected).
      const treasuryAddress = process.env.TREASURY_ADDRESS as
        | Address
        | undefined;
      const networkFeeUsd = treasuryAddress ? quote.networkFeeUsd : 0;
      ...
      if (isAvax) {
        ...
        const feeAvaxAmount =
          networkFeeUsd > 0 ? networkFeeUsd / quote.tokenPriceUsd! : 0;
        const balanceRaw = await getAvaxBalance(
          sender.walletAddress as Address,
        );
        if (
          balanceRaw < parseUnits((avaxAmount + feeAvaxAmount).toFixed(18), 18)
        ) {
        ...
        const requiredRaw = parseUnits(
          (amountUsd + networkFeeUsd).toFixed(6),
          6,
        );
      ...
      if (isTumaUser) {
        ...
        try {
          ...
          const feeUsd = isMerchantPayment
            ? parseFloat(
                ((amountUsd * recipientMerchant!.feeBps) / 10_000).toFixed(6),
              )
            : 0;
          ...
          if (isMerchantPayment && feeUsd > 0 && treasuryAddress) {
            ...
            feePromise.catch((err) =>
              console.error(
                `[Send] Merchant fee transfer failed for ${reference}:`,
                err.message,
              ),
            );
          }
          ...
          if (networkFeeUsd > 0 && treasuryAddress) {
            ...
            networkFeePromise.catch((err) =>
              console.error(
                `[Send] Network fee transfer failed for ${reference}:`,
                err.message,
              ),
            );
          }
          ...
          if (isAvax) {
            ...
            await recordSettlementStep(tx.id, "settled", {
              txHash,
              reason: "AVAX settles directly to recipient wallet",
            });
        ...
        try {
          ...
          // Separate leg from the escrow deposit above — charged on the
          // sender's side regardless of the recipient claiming later.
          if (networkFeeUsd > 0 && treasuryAddress) {
            transferToken(
              escrowToken,
              recipientHash,
