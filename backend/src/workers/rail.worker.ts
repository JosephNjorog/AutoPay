...
import { queueConnectionOptions, QUEUE_NAMES, type RailDisburseJob } from "../lib/queue";
...
if (worker) {
  ...
  console.warn("[RailWorker] REDIS_URL not set — rail disbursement worker disabled");
  void recordHeartbeat({
    component: "rail.worker",
    kind: "worker",
    status: "error",
