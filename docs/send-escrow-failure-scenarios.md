...
| --- | --- | --- | --- | --- | --- |
| Client retry | Same `/api/send` retried after timeout | `idempotencyKey` replays existing transaction for the same sender. In-flight duplicate returns `409`; both paths have Postgres/Redis integration coverage. | Implemented | Only works if the client reuses the same key. | Generate and persist idempotency keys in the frontend send flow. |
...
| Escrow deposit | Deposit succeeds but DB update or escrow row insert fails | `escrow.worker` scans `Deposited` events with a persistent chain cursor. If the local transaction already has the escrow ref, the scanner can attach the tx hash, rebuild a missing escrow row, and reschedule expiry; this repair path has integration coverage. Operators can still attach a confirmed chain hash manually. | Partial | If PostgreSQL was unavailable before `escrowRef` was written to the transaction, the chain event cannot be tied back to local intent without an external lookup. | Persist the escrow ref before broadcast or add an outbox/prepared-send record. |
| Expiry scheduling | Redis queue disabled, schedule call returns false, or delayed job is lost | Escrow worker scans expired pending escrows and either re-enqueues a deterministic expiry job or processes inline when the queue is unavailable. The re-enqueue path has integration coverage and uses a BullMQ-safe deterministic job id. | Implemented | Recovery depends on the escrow worker running and being able to reach the database. | Wire an external heartbeat monitor. |
| Claim-link notification | Queue/send fails after funds escrowed | Transaction is marked `requires_review`; operators can resend the claim link through `/api/ops/review/:transactionId/resend-claim-link`, with integration coverage for queued resend recovery. | Implemented | The recovery action is API-level; production still needs alerts and a dashboard. | Add notification failure dashboard and SLA alerts. |
...
| On-chain claim | Claim succeeds | Escrow row stores `claimTxHash`, claiming wallet, and claim timestamp. Transaction records on-chain claim event; duplicate replay and scanner repair have integration coverage. | Implemented | Full chain execution still belongs in contract-backed end-to-end tests. | Add contract-backed on-chain claim flow coverage. |
| On-chain claim | Claim succeeds but DB update fails | The claim route records `escrow_claim_db_update` review metadata when it can. If PostgreSQL was unavailable before that metadata was written, `escrow.worker` scans `Claimed` events and replays local claim persistence plus rail handoff from the contract event and local escrow row. Both metadata replay and chain-event repair have integration coverage. | Implemented | Requires the scanner cursor to run and the claimed wallet to map to a local user. | Add scanner dashboard panels and alert correlation. |
...
| Duplicate claim tap | Recipient submits claim twice quickly | Claim submission uses a short escrow-ref lock; once claimed, the same recipient gets an idempotent replay response instead of a second chain attempt. Replay and held-lock conflict paths have integration coverage. | Implemented | The lock is best-effort and TTL-based, so the on-chain contract remains the final duplicate-claim guard. | Add a lock TTL-expiry edge-case test if the claim path becomes injectable. |
...
| Expiry worker | Refund transaction fails | Worker retries with BullMQ backoff and marks `requires_review` after final retry failure; operators can retry the refund through `/api/ops/review/:transactionId/refund-escrow`. Unsafe refund retry rejection has integration coverage. | Implemented | On-chain expiry and pending-state checks still decide whether a refund is valid; success-path retry still needs a mockable chain boundary or contract-backed test. | Add success-path refund retry coverage and operator runbook. |
| Expiry worker | Refund succeeds but DB update or review metadata write fails | `escrow.worker` scans `Refunded` events and repairs local escrow/transaction status to refunded/expired, with integration coverage. | Implemented | Requires the local escrow row to exist and the chain-event cursor to advance. | Add scanner dashboard panels and alert correlation. |
| Missed expiry | Queue job was never scheduled, Redis lost data, or worker was down long-term | Periodic scanner finds `pending` escrows with `expiresAt < now` and repairs by enqueuing or inline processing. The re-enqueue and missing-sender review paths have integration coverage. | Implemented | Scanner must be running; deterministic job ids avoid duplicate queue buildup but do not replace monitoring. | Wire an external heartbeat monitor. |
...
| Integration tests | Resilience path regressions reach review/queue/scanner code | Bun integration tests run against real Postgres and Redis, migrate schema first, reset DB/Redis between specs, and cover heartbeat stale/fail behavior, rail dead-letter retry, duplicate send/claim idempotency, expiry scanner repair, claim DB reconciliation, chain-event deposit/claim/refund repair, operator claim-link resend, operator chain-hash reconciliation, and unsafe refund retry rejection. | Implemented | Tests are route/service-level; they do not call real Avalanche RPC endpoints or payment-provider sandboxes. | Add contract-backed chain success paths and provider sandbox checks. |
