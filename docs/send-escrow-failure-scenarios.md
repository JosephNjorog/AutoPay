...
| Expiry worker | Refund succeeds but DB update or review metadata write fails | `escrow.worker` scans `Refunded` events and repairs local escrow/transaction status to refunded/expired. | Implemented | Requires the local escrow row to exist and the chain-event cursor to advance. | Add scanner heartbeat/alerting and refund event tests. |
