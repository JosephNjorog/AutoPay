...
function Receive() {
  ...
  return (
    <PageFrame sidebar maxWidth="narrow">
      <div className="flex min-h-full flex-col p-5 font-manrope">
        ...
        <div className="mt-6 relative">
          <div className="absolute -inset-4 rounded-[2rem] opacity-30 blur-2xl bg-ink" />
          <div className="relative rounded-[2rem] p-6 text-paper bg-ink">
            ...
            <div className="mt-5 rounded-2xl bg-paper p-5">
              {qrDataUrl
                ...
                : <div className="w-full aspect-square animate-pulse bg-ink/8 rounded-xl" />}
            </div>
            <div className="mt-5">
        ...
        <div className="mt-5 space-y-2">
          <button
            ...
            className="w-full flex items-center justify-between rounded-2xl bg-paper border border-ink/10 p-4"
          >
            <div className="text-left">
              <p className="text-[10px] uppercase tracking-wider text-slate">Phone number</p>
              <p className="text-sm font-bold">{phone}</p>
            </div>
            <div className="h-9 w-9 rounded-full bg-amber/16 text-amber-deep flex items-center justify-center">
              {copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
            </div>
          </button>

          <div className="flex items-center justify-between gap-2 rounded-2xl bg-paper border border-ink/10 p-4">
            <div className="text-left min-w-0">
              <p className="text-[10px] uppercase tracking-wider text-slate">Smart wallet</p>
              {smartWallet
                ? <p className="text-sm font-mono truncate">{smartWallet.slice(0,12)}…{smartWallet.slice(-6)}</p>
                : <p className="text-sm text-slate">Deploying…</p>
              }
            </div>
            <div className="flex items-center gap-1 shrink-0">
              {smartWallet && (
                <button
                  ...
                  className="h-9 w-9 rounded-full bg-amber/16 text-amber-deep flex items-center justify-center"
                >
                  {copiedWallet ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                </button>
              )}
              <Link to="/wallet" className="text-xs text-forest font-semibold px-2">View</Link>
            </div>
          </div>
        </div>

        <p className="mt-5 text-center text-[11px] text-slate">Anyone can scan this QR or send to your number — even if they don't use Autopayke.</p>
      </div>
    </PageFrame>
  );
}
